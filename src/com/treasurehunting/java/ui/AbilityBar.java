...
public class AbilityBar {
    ...
    private Sprite abilityBackground = Assets.abilityBoardSS.getSprite(0, 0);
    private HashMap<Integer, AbilityButton> buttons = new HashMap<>();

    public AbilityBar() {
        abilityBackground.setResizeImage((int) (abilityBackground.getWidth()*2), (int) (abilityBackground.getHeight()*2));
    ...
    public void render(Graphics2D g2d) {
        g2d.drawImage(abilityBackground.image, GameSettings.GAME_WIDTH / 2 - abilityBackground.getWidth() / 2, GameSettings.GAME_HEIGHT - 90 - abilityBackground.getHeight() / 2, abilityBackground.getWidth(), abilityBackground.getHeight(), null);
        for (Map.Entry<Integer, AbilityButton> item : buttons.entrySet()) {
